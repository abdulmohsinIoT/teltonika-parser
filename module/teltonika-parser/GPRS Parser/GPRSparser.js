...
const calcCRC16 = (str) => CRC16_1.CalcCRC16(str).toString(16).padStart(8, '0');
function gerateCodec12(command) {
    command = stringUtil_1.toHex(command);
...
function parseCodec12(hexStr) {
    let [preamble, content, crc] = stringUtil_1.splitAt(hexStr, 8, -8);
    if (preamble !== ''.padStart(8, '0'))
        throw new Error(`Invalid preamble.`);
    let [dataSize, data] = stringUtil_1.splitAt(content, 8);
