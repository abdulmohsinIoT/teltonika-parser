...
const avlKeyDict = require("./General_IOElement_Parser/avlKeyDict");
const avlValueDict = require("./General_IOElement_Parser/avlValueDict");
...
function castAVLIDtoAVLName(elements = null) {
    ...
    for (var i = 0; i < keys.length; i++) {
        ...
        if (!avlDict_1.avlidDictionary.hasOwnProperty(id))
            continue;
        var translated = avlDict_1.avlidDictionary[Number(id)];
        if (translated == null)
            continue;
...
function processIOElements (array) {
    array = _.mapKeys(array, (value, key) => avlKeyDict.dict[key])

    delete array.undefined;

    array = _.mapValues(array, (value, key) => avlValueDict.dict[key](value))
...
class IOelement {
    constructor(reader, on_error, codec_id) {
        ...
        this.ParsedElements = processIOElements(this.Elements);
    }
}
